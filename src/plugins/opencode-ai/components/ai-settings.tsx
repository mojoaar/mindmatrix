...
import { GO_DOCS_URL } from "../constants";
...
export function OpenCodeSettings({ workspaceId }: { workspaceId: string }) {
  ...
  useEffect(() => {
    fetch("/api/plugins/opencode-ai/models")
      .then((r) => r.json())
      .then((d) => { if (d.models) setModels(d.models); })
      .catch(() => {});
    fetch(`/api/plugins/config?pluginId=opencode-ai&workspaceId=${workspaceId}`)
      .then((r) => r.json())
      .then((d) => {
        if (d.config?.config) {
  ...
  return (
    <div>
      ...
      <div className="flex align-center justify-between gap-2">
        ...
        <a href={GO_DOCS_URL} target="_blank" rel="noopener" className="text-xs text-muted">
          OpenCode Go docs →
        </a>
      </div>
